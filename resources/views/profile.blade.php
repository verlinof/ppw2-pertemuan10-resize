    ...
    <div class="flex flex-col items-center pb-10">
        ...
        <div class="flex mt-4 space-x-3 md:mt-6">
            ...
            <form action="{{ route('delete-account') }}" method="POST">
                @csrf
                @method('DELETE')
                <button onclick="return confirm('Apakah anda yakin ingin menghapus akun ini?')" type="submit" href="#" class="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white bg-red-600 border border-gray-300 rounded-lg hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-gray-200">Delete</button>
            </form>
        </div>
    </div>
</div>
